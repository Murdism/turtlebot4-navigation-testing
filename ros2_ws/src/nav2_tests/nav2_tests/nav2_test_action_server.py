...
class Nav2TestActionServer(Node):
    ...
    def __init__(self):
        super().__init__('nav2_test_action_server')
        ...
        self.declare_parameter('entity_name', 'turtlebot4_robot')
    ...
    def plan_callback(self, msg):
        ...
        # Note: We ignore subsequent plan updates to keep the original full path
    ...
    def set_initial_pose(self, x, y, z, yaw):
        ...
        cmd = [
            "ros2", "run", "nav2_tests", "initial_pose_publisher",
            "--ros-args",
            "-p", f"x:={x}",
            "-p", f"y:={y}",
            "-p", f"z:={z}",
    ...
    def save_summary_report(self, test_name, repetitions):
        ...
        summary = {
            ...
            "timing_statistics": {
                ...
                "time_std_dev_s": round(self._calculate_std_dev(self.run_durations), 2) if len(self.run_durations) > 1 else 0.0,
                "all_durations_s": [round(d, 2) for d in self.run_durations]
            },
